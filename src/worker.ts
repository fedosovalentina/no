import { detectLocaleFromHeader } from "./locale-detect";
import { getEmbedPhrase, pickRandomEmbedIndex } from "./embed-phrases";
...
const OG_CACHE = "public, max-age=300";
...
function ogImageResponse(request: Request): Response {
  const url = new URL(request.url);
  const langParam = url.searchParams.get("lang");
  const locale =
    langParam === "ru" || langParam === "en"
      ? langParam
      : detectLocaleFromHeader(request.headers.get("Accept-Language"));

  const rawIndex = url.searchParams.get("i");
  let index = rawIndex !== null ? parseInt(rawIndex, 10) : pickRandomEmbedIndex(locale);
  if (Number.isNaN(index)) index = pickRandomEmbedIndex(locale);

  const compact = url.searchParams.get("compact") === "1";
  const hidePhrase = url.searchParams.get("hide") === "1";
  const phrase = getEmbedPhrase(locale, index);
  const svg = buildOgSvg(phrase, { compact, hidePhrase });
...
async function serveAsset(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "/" || path.endsWith(".html")) {
    ...
    const phraseIndex = pickRandomEmbedIndex(locale);
    const origin = new URL(request.url).origin;
    const body = path === "/" ? injectOgMeta(html, { locale, phraseIndex, origin }) : html;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (pathname === "/og.svg" || pathname === "/og.jpg" || pathname === "/og.png" || pathname === "/og") {
